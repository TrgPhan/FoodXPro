...
async def get_daily_meals(
    ...
    for meal in user_meals:
        ...
        if meal.eat_at == 'breakfast':
            breakfast.append({"id": recipe_id, "name": recipe_name, "image": recipe_image, "servings_eaten": servings_eaten})
        elif meal.eat_at == 'lunch':
            lunch.append({"id": recipe_id, "name": recipe_name, "image": recipe_image, "servings_eaten": servings_eaten})
        elif meal.eat_at == 'dinner':
            dinner.append({"id": recipe_id, "name": recipe_name, "image": recipe_image, "servings_eaten": servings_eaten})
        else:
            snack.append({"id": recipe_id, "name": recipe_name, "image": recipe_image, "servings_eaten": servings_eaten})
...
async def add_daily_meals(
    ...
    user_meal_result = user_meal_result.scalar_one_or_none()
    if user_meal_result:
        raise HTTPException(
            status_code = 409,
            detail="meal already added"
        )
