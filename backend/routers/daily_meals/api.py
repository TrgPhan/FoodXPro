...
async def get_daily_meals(
    ...
    result = await db.execute(
        ...
        .options(
            ...
            selectinload(UserMeals.recipe).selectinload(
                Recipes.recipe_has_nutrition),
            selectinload(UserMeals.recipe).selectinload(
                Recipes.recipe_has_nutrition).selectinload(RecipeHaveNutrition.contains_nutrition)
        )
    ...
    for meal in user_meals:
        ...
        if meal.eat_at == 'breakfast':
            breakfast.append({"id": recipe_id, "name": recipe_name, "image": recipe_image,
                             "servings_eaten": servings_eaten, "calories": calories})
...
async def add_daily_meals(
    ...
    servings_eaten: Annotated[int, Query(
        description="number of servings eaten")] = 1
):
    ...
    ingredient_ids = [
        user_ingredient.ingredient_id for user_ingredient in user_ingredients]
