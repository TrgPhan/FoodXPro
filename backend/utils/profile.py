...
def parse_adjusted_value_details(adjusted_value: str, nutrition: str) -> Dict[str, Any]:
        ...
        if range_match:
            ...
            details["unit"] = range_match.group(3) if range_match.group(3) else None
            details["value"] = (details["min_value"] + details["max_value"]) / 2  # Average for compatibility
        elif numeric_match:
            # Extract single numeric value and unit
            # Pattern for values like "20%", "1500mg", "0.8g/kg"
            details["value"] = float(numeric_match.group(1))
            details["unit"] = numeric_match.group(2) if numeric_match.group(2) else None
...
def calculate_adjusted_value(user: users.Users, user_nutritions_goal: Dict[str, float], affected_nutritions: Dict[str, str]) -> Dict[str, float]:
    for nutrition, adjusted_value in affected_nutritions.items():
        parsed_adjusted_value = parse_adjusted_value_details(adjusted_value, nutrition)
        ...
        if parsed_adjusted_value["direction"] == "increase":
            if parsed_adjusted_value["unit"] == "%":
                user_nutritions_goal[nutrition] = user_nutritions_goal[nutrition] * (1 + parsed_adjusted_value["value"] / 100)
            elif parsed_adjusted_value["unit"] == "g/kg":
                user_nutritions_goal[nutrition] = user_nutritions_goal[nutrition] + parsed_adjusted_value["value"] * user.weight
            else: 
                user_nutritions_goal[nutrition] = user_nutritions_goal[nutrition] + parsed_adjusted_value["value"]
            ...
            if parsed_adjusted_value["unit"] == "%":
                if 'Fat' in nutrition:
                    user_nutritions_goal[nutrition] = (parsed_adjusted_value["value"] / 100 * user_nutritions_goal["Calories"]) / CONVERT_TO_CALORIES['Fat']
                elif 'Carbs' in nutrition:
                    user_nutritions_goal[nutrition] = (parsed_adjusted_value["value"] / 100 * user_nutritions_goal["Calories"]) / CONVERT_TO_CALORIES['Carbs']
                elif 'Protein' in nutrition:
                    user_nutritions_goal[nutrition] = (parsed_adjusted_value["value"] / 100 * user_nutritions_goal["Calories"]) / CONVERT_TO_CALORIES['Protein']
            else:
...
def calculate_macors_intake(weight, height, age, sex, activity_level, goal):
    ...
    if activity_level is None:
        raise ValueError("activity_level cannot be None, please update your profile")
    if goal is None:
    ...
    carbohydrates_intake = calorie_intake * EATING_GOALS[goal]["carbohydrates"] / 4
    return {
